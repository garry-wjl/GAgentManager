...
### 1.4 成功定义
- 用户能够在一个平台上管理所有Agent、模型、Skill、MCP、用户和权限
- Agent发布时间缩短60%
- Agent运行效率提升40%
- 用户满意度达到85%以上
...
| roleNames | Array | 是 | 用户关联的角色列表，通过权限管理模块配置，此处仅做展示 |
...
| roleNames | Array | 否 | 角色列表，通过权限管理模块配置，创建时可预设 |
| notes | Text | 否 | 备注信息，最大500字符 |
...
| roles | Array | 否 | 组关联的角色ID列表，通过权限管理模块配置 |
| createTime | DateTime | 是 | 创建时间 |
| creator | String | 是 | 创建人 |
...
- 角色关联：通过权限管理模块为用户分配或移除角色（详见4.1.4 权限管理）
- 单点登录（SSO）：支持企业级SSO集成
- 用户行为审计：记录所有用户操作日志
- 密码策略管理：支持复杂度要求和定期更换
- 多因素认证：支持双因子认证
...
#### 4.1.4 权限管理
**功能描述：** 基于RBAC（Role-Based Access Control）模型，提供完整的角色、权限、用户关联管理能力，实现细粒度的访问控制
**用户故事：** 作为系统管理员，我希望能够通过角色来管理用户权限，给不同角色配置不同的用户，以便高效、统一地控制用户对系统各模块的访问权限。

**权限操作功能：**

- **角色新增**：创建新角色，定义角色名称、描述和权限集合
- **角色修改**：编辑角色的基本信息和权限配置
- **角色删除**：删除不再使用的自定义角色，系统内置角色不可删除
- **角色启用/禁用**：控制角色是否可用于新用户分配，禁用后已关联用户不受影响
- **权限分配**：为角色分配功能模块的操作权限
- **用户关联**：将用户关联到指定角色，实现批量权限授予
- **用户移除**：从角色中移除指定用户，移除后用户失去该角色的权限

**角色列表字段设计：**

| 字段名 | 字段类型 | 是否必填 | 说明/约束 |
|--------|----------|----------|-----------|
| roleId | String | 是 | 角色唯一标识，系统自动生成（UUID） |
| roleCode | String | 是 | 角色编码，字母数字下划线，全局唯一，如admin、developer |
| roleName | String | 是 | 角色名称，2-50字符，全局唯一 |
| description | Text | 否 | 角色描述，最大500字符 |
| isSystem | Boolean | 是 | 是否系统内置角色，内置角色不可删除 |
| isEnabled | Boolean | 是 | 是否启用，默认true，禁用后不可用于新用户分配 |
| userCount | Number | 是 | 关联用户数量，只读 |
| permissionCount | Number | 是 | 关联权限数量，只读 |
| creator | String | 是 | 创建人，关联用户ID |
| createTime | DateTime | 是 | 创建时间 |
| updater | String | 是 | 更新人，系统自动记录最后修改人 |
| updateTime | DateTime | 是 | 更新时间 |
...
| 字段名 | 字段类型 | 是否必填 | 说明/约束 |
|--------|----------|----------|-----------|
| roleCode | String | 是 | 角色编码，字母开头，仅字母数字下划线，3-30字符，不可重复 |
| roleName | String | 是 | 角色名称，2-50字符，不可重复 |
| description | Text | 否 | 角色描述，最大500字符 |
| permissions | Array | 是 | 权限列表，选择该角色拥有的所有权限项 |
...
| 字段名 | 字段类型 | 是否必填 | 说明/约束 |
|--------|----------|----------|-----------|
| actionId | String | 是 | 操作唯一标识，系统自动生成 |
| actionCode | String | 是 | 操作编码，如read、write、delete、admin |
| actionName | String | 是 | 操作名称，如查看、新增/修改、删除、管理 |
...
| 字段名 | 字段类型 | 是否必填 | 说明/约束 |
|--------|----------|----------|-----------|
| rolePermissionId | String | 是 | 关联唯一标识，系统自动生成 |
| roleId | String | 是 | 角色ID |
| resourceId | String | 是 | 资源ID |
| actionId | String | 是 | 操作ID |
| permissionCode | String | 是 | 权限编码，格式为"resourceCode:actionCode"，如agent:read |
| grantType | Enum | 是 | 授权类型：允许、拒绝，用于精细化控制 |
| createTime | DateTime | 是 | 创建时间 |
...
| 字段名 | 字段类型 | 是否必填 | 说明/约束 |
|--------|----------|----------|-----------|
| userRoleId | String | 是 | 关联唯一标识，系统自动生成 |
| userId | String | 是 | 用户ID |
| roleId | String | 是 | 角色ID |
| assignType | Enum | 是 | 分配方式：直接分配、继承（从用户组继承） |
| assignTime | DateTime | 是 | 分配时间 |
| assignUser | String | 是 | 分配操作人 |
| expireTime | DateTime | 否 | 过期时间，空表示永久有效 |
...
| directRoles | Array | 是 | 直接关联的角色列表 |
| inheritedRoles | Array | 是 | 从用户组继承的角色列表 |
| allPermissions | Array | 是 | 汇总后的权限编码列表（去重） |
| deniedPermissions | Array | 是 | 被拒绝的权限列表（优先级高于允许） |

**角色权限配置界面字段设计：**

| 字段名 | 字段类型 | 是否必填 | 说明/约束 |
|--------|----------|----------|-----------|
| moduleCode | String | 是 | 模块编码，如agent、user、skill |
| moduleName | String | 是 | 模块名称，如Agent管理、用户管理 |
| actions | Array | 是 | 该模块下各操作的权限状态（允许/拒绝/未设置） |
| isChecked | Boolean | 是 | 模块全选状态 |
| children | Array | 否 | 子资源权限列表（支持树形结构） |
...
| department | String | 否 | 所属部门 |
| assignType | Enum | 是 | 分配方式：直接分配、继承 |
| assignTime | DateTime | 是 | 分配时间 |
| expireTime | DateTime | 否 | 过期时间 |
| operator | String | 是 | 操作列：移除按钮 |

**具体需求：**
- RBAC模型：采用"用户-角色-权限"三层模型，用户通过角色获得权限
- 角色分类：系统内置角色（超级管理员、管理员、开发者、数据科学家、普通用户、访客）不可删除，自定义角色可增删改
- 权限粒度：支持模块级、菜单级、按钮级、接口级权限控制
- 权限编码规范：格式为"resourceCode:actionCode"，如agent:read、user:write、skill:delete
- 权限继承：支持通过用户组继承角色权限
- 权限拒绝：支持在角色中设置"拒绝"权限，优先级高于"允许"
- 临时角色：支持为角色设置过期时间，到期后自动失效
- 权限缓存：权限数据缓存至Redis，减少数据库查询
- 权限实时刷新：用户权限变更后实时清除缓存，下次请求生效
- 权限审计：记录所有角色和权限的变更操作
- 批量用户关联：支持一次将多个用户关联到指定角色
- 角色复制：支持基于现有角色快速创建新角色
- 权限可视化配置：以树形矩阵方式展示和配置权限

**验收标准：**
- 支持系统内置6种预设角色和自定义角色的增删改查
- 可以为角色配置任意模块的任意操作权限
- 支持将用户关联到角色，也支持从角色中移除用户
- 支持批量将多个用户关联到同一角色
- 权限变更后用户在下次请求时立即生效
- 权限查询响应时间小于100毫秒
- 所有角色和权限变更操作均有审计日志记录
...
1. 首页（4.1.3）
   - 系统概览卡片（Agent总数、在线用户数、活跃Agent数等）
   - 快捷操作入口（创建Agent、安装Skill等）
   - 通知和提醒中心

2. Agent管理（4.1.1）
   - Agent列表（分页、筛选、搜索）
   - Agent详情查看
   - Agent操作：新增、修改、发布、上线、下线、删除
   - 版本管理：版本历史查看、版本回滚、版本对比、分支管理
   - Agent基础配置：名称、描述、图标、管理员、模型绑定、参数配置
   - Skill配置：已安装Skill绑定、启用/禁用、优先级设置、参数配置
   - MCP配置：已配置MCP服务绑定、启用/禁用、连接参数设置
   - 工作流管理：在Agent内创建/修改/发布/下线/删除工作流，工具化配置，参数传递

3. 用户管理（4.1.2）
   - 用户列表（分页、筛选、搜索）
   - 用户详情查看
   - 用户操作：新增、启用、禁用、删除、离职、重置密码
   - 用户批量操作：批量启用/禁用/删除
   - 用户组管理：创建/编辑/删除用户组，添加/移除组成员
   - 用户操作日志查看

4. 权限管理（4.1.4）
   - 角色列表（分页、筛选、搜索）
   - 角色操作：新增、修改、删除、启用/禁用、复制
   - 权限矩阵配置：以树形矩阵方式配置角色权限（模块级/菜单级/按钮级/接口级）
   - 用户-角色关联：查看角色下关联用户，将用户关联到角色，从角色中移除用户
   - 批量用户关联：一次将多个用户关联到指定角色
...
6. MCP管理（4.1.6）
   - MCP服务列表（分页、筛选、搜索）
   - MCP服务详情查看
   - MCP操作：新增、删除、启用、禁用、修改
   - MCP版本控制：版本历史查看、最新版本自动生效
...
| searchScope | Array | 是 | 搜索范围：Agent、Skill、MCP、模型、工作流、用户、权限、角色 |
...
| 字段名 | 字段类型 | 是否必填 | 说明/约束 |
|--------|----------|----------|-----------|
| operationType | Enum | 是 | 操作类型：启用、停用、删除、导出、关联角色 |
...
- **用户组表：** 用户分组信息
- **用户组关联表：** 用户与用户组的关联关系
- **角色表：** RBAC角色定义（名称、编码、描述、内置标识、启用状态）
- **权限资源表：** 权限资源定义（模块、菜单、按钮、接口，支持树形层级）
- **权限操作表：** 操作定义（查看、新增、修改、删除、管理等）
- **角色权限关联表：** 角色与权限的多对多关系（资源+操作，允许/拒绝）
- **用户角色关联表：** 用户与角色的多对多关系（直接分配、用户组继承、过期时间）
- **权限模板表：** 预设权限模板，用于快速创建角色
- **用户操作日志表：** 详细记录用户操作
...
- Agent管理功能开发
- RBAC权限管理功能开发（角色增删改查、权限矩阵配置、用户角色关联）
- 用户管理功能开发
- 个性化首页开发
- 模型基础功能开发
- MCP基础功能
...
**交付物：**
- Agent CRUD功能
- RBAC权限管理系统（角色管理、权限配置、用户关联、批量操作、权限缓存）
- 用户管理系统
- 首页仪表盘
